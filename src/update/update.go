...
// SetUpdateChannel persists the update channel preference to config file
// Per AI.md PART 23 specification
func SetUpdateChannel(channel UpdateChannel, configDir string) error {
	// Validate channel
	if channel != ChannelStable && channel != ChannelBeta && channel != ChannelDaily {
		return fmt.Errorf("invalid channel: %s", channel)
	}

	// This would require importing config package, which creates a circular dependency
	// Instead, we'll write directly to server.yml
	...
	// Simple YAML update: look for update_channel line and replace it
	lines := strings.Split(string(data), "\n")
	found := false
	for i, line := range lines {
		if strings.HasPrefix(strings.TrimSpace(line), "update_channel:") {
			lines[i] = fmt.Sprintf("update_channel: %s", channel)
			found = true
			break
		}
	}
	...
	if !found {
		lines = append(lines, fmt.Sprintf("update_channel: %s", channel))
	}
